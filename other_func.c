...
/**
 * print_decimal_hex - prints a decimal in hexadecimal
...
int print_decimal_hex(va_list arguments, char *buf, unsigned int ibuf)
{
	...
	hexadecimal = _hex_array(binary, hexadecimal, 1, 8);
...
/**
 * string_values - prints a string and values of
 * non-printed chars
 * @arguments: input string
 * @buf: buffer pointer
 * @ibuf: index for buffer pointer
...
int string_values(va_list arguments, char *buf, unsigned int ibuf)
{
	...
	for (sum = i = 0; str[i]; i++)
	{
		if (str[i] < 32 || str[i] >= 127)
		{
			...
			op = str[i];
			binary = _binary_array(binary, op, 0, 32);
			hexadecimal = _hex_array(binary, hexadecimal, 1, 8);
